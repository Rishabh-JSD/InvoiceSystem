...
//	}
	public Custentity addcustomer(Custentity cust) {
        ...
        return null; // Customer with the same name already exists
    }
	 public boolean existsByCustomerName(String customerName) {
	        try {
	            entityManager.createQuery("SELECT c FROM Custentity c WHERE c.customerName = :customerName", Custentity.class)
	                    .setParameter("customerName", customerName)
	                    .getSingleResult();
	            return true; // Customer name exists
	        } catch (NoResultException ex) {
	            return false; // Customer name does not exist
	        }
	    }
}
